...
import com.zybooks.studyhelper.StudyFetcher.OnStudyDataReceivedListener
    ...
    private val fetchListener = object : OnStudyDataReceivedListener {
        ...
        override fun onQuestionsReceived(subject: Subject, questionList: List<Question>) {
            if (questionList.isNotEmpty()) {
                ...
                Toast.makeText(applicationContext, "${subject.text} imported successfully",
                    Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(applicationContext, "${subject.text} contained no questions",
                    Toast.LENGTH_SHORT).show()
            }
        }
